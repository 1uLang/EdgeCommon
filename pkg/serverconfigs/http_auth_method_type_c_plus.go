...
// HTTPAuthTypeCMethod URL鉴权方式C
// https://domainName/hash/timestamp/path
// Hash算法：HASH(path@timestamp@secret)
// 回源URL：https://domainName/path
type HTTPAuthTypeCMethod struct {
	HTTPAuthBaseMethod
...
func NewHTTPAuthTypeCMethod() *HTTPAuthTypeCMethod {
	return &HTTPAuthTypeCMethod{
...
// Init 初始化
func (this *HTTPAuthTypeCMethod) Init(params map[string]any) error {
...
// Filter 过滤
func (this *HTTPAuthTypeCMethod) Filter(req *http.Request, subReqFunc func(subReq *http.Request) (status int, err error), formatter func(string) string) (ok bool, newURI string, uriChanged bool, err error) {
	var life = this.Life
	if life <= 0 {
		life = 30
	}
